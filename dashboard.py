...
const path=location.pathname,fmt=t=>t?new Date(t*1000).toLocaleString():'-',cls=s=>s==='OK'||s==='active'?'ok':(s==='DEGRADED'||s==='missing'?'warn':'bad');async function api(p,o){let r=await fetch(p,o);return await r.json()}function card(t,v,c=''){return `<div class="card"><div class="muted">${t}</div><div class="big ${c}">${v??'-'}</div></div>`}function bars(h,k){return `<div class="bar">${h.slice(-80).map(x=>`<span title="${k}: ${x[k]??'-'}" style="height:${Math.max(2,Math.min(100,Number(x[k]||0)))}%"></span>`).join('')}</div>`}
async function draw(){let s=await api('/api/status'),h=await api('/api/history'),e=await api('/api/events'),app=document.getElementById('app'),health=s.health||{},m=s.metrics||{},n=s.network||{};if(path==='/history'){app.innerHTML=`<div class="grid"><div class="card"><b>CPU</b>${bars(h,'cpu')}</div><div class="card"><b>RAM</b>${bars(h,'ram')}</div><div class="card"><b>RSSI</b>${bars(h,'rssi')}</div><div class="card"><b>Gateway Ping</b>${bars(h,'gateway_ms')}</div><div class="card"><b>Internet Ping</b>${bars(h,'internet_ms')}</div><div class="card"><b>Health</b>${bars(h,'health')}</div></div>`;return}if(path==='/service'){app.innerHTML=`<div class="card"><b>System Control</b><p class="muted">Allowlist only. No arbitrary shell execution.</p><button onclick="act('netwatchdog-oled')">Restart OLED</button><button onclick="act('netwatchdog')">Restart Status Writer</button><button onclick="act('netwatchdog-dashboard')">Restart Dashboard</button><pre id="out"></pre></div><div class="card"><b>Services</b><table>${Object.entries(s.services||{}).map(([k,v])=>`<tr><td>${k}</td><td class="${cls(v)}">${v}</td></tr>`).join('')}</table></div>`;return}if(path==='/advanced'){app.innerHTML=`<div class="grid">${card('Version',s.version)}${card('Git Commit',s.git_commit)}${card('Uptime',Math.round((s.uptime_sec||0)/60)+' min')}${card('Disk',m.disk+'%')}</div><div class="card"><b>Raw Status</b><pre>${JSON.stringify(s,null,2)}</pre></div>`;return}app.innerHTML=`<div class="grid">${card('Health',health.score+'%',cls(health.status))}${card('Status',health.status,cls(health.status))}${card('Active Route',s.active)}${card('CPU',m.cpu+'%')}${card('RAM',m.ram+'%')}${card('Temp',m.temp+'°C')}${card('RSSI',(n.usb&&n.usb.signal_dbm)||'-')}${card('Internet',n.internet_ms===null?'Lost':n.internet_ms+' ms',n.internet_ms===null?'bad':'ok')}</div><div class="grid"><div class="card"><b>Health Detail</b><ul>${(health.reasons||[]).map(x=>`<li>${x}</li>`).join('')}</ul></div><div class="card"><b>Recent Events</b><table>${e.slice(-10).reverse().map(x=>`<tr><td>${fmt(x.ts)}</td><td>${x.event}</td><td class="muted">${x.detail||''}</td></tr>`).join('')}</table></div></div>`}
